...
import {
  useSendSalonOtp,
  useVerifySalonOtp,
} from "./SalonLogin/hooks/useSalonLogin";
...
export default function SalonLogin() {
  ...
  const [activeMethod, setActiveMethod] = React.useState<AuthMethod | null>(
    null,
  );
  ...
  const handleVerifyOTP = async (otp: string) => {
    ...
    try {
      ...
      if (response.success) {
		...
		console.log("OTP verification successful, logging in user with phone:", response?.data?.phone);
      ...
      notifyError(
        response.message ?? "Could not verify OTP. Please try again.",
      );
    } catch (err: any) {
